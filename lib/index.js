...
var consumare = {
  ...
  meta: function(filePath) {
    var filePromise = storage.file(filePath);

    // Once completed, return the parsed document contents.
    return Promise.resolve(filePromise).then(function(contents) {
  ...
  assemble: function(filePath, rev, callback) {
    ...
    return Promise.resolve(filePromise).spread(function(contents, revs) {
      ...
      function highlight(val, lang) {
        ...
        var codeBlock = "<pre><div class=\"code\"><code>";
        ...
        codeBlock = codeBlock.replace(/hljs-/g, "");
